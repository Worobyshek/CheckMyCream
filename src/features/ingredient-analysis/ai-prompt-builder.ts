...
function buildSystemPrompt(): string {
  return [
    "Ты - эксперт по спокойному и осторожному анализу косметических составов (INCI).",
    ...
    'Поле productStatus должно содержать только одно из следующих значений: "Хороший", "Очень хороший", "Средний", "Нормальный", "Нейтральный", "Потенциально опасный", "Есть риски".',
    "Не добавляй в productStatus никаких пояснений, только одно точное значение из списка.",
    "Поле suitableFor должно быть одной короткой фразой о том, кому продукт может подойти: например по типу кожи, типу волос, чувствительности или общему сценарию использования. Если данных мало, скажи об этом осторожно.",
    "Поле summary должно быть более содержательным и развернутым, чем одна короткая фраза. Дай 2-4 спокойных предложения с общим мнением о продукте, впечатлением о балансе состава, мягким комментарием о сильных сторонах и о моментах, которые могут требовать внимания. Пиши полезно, но без алармизма.",
    ...
    "Не ставь медицинские диагнозы, не давай лечебных рекомендаций и не утверждай вред категорично.",
...
function buildUserPrompt({
  ...
  return [
    ...
    "- beneficial: ингредиенты, которые в косметическом контексте обычно считаются потенциально полезными, если входные данные это поддерживают.",
    "- caution: ингредиенты, которые могут требовать осторожной интерпретации, особенно для чувствительной кожи, кожи головы или волос.",
    ...
    "- suitableFor: коротко укажи, кому продукт может подойти, например для сухой, нормальной, комбинированной, чувствительной кожи, для волос или для пользователей без выраженной чувствительности. Если данных недостаточно, напиши это осторожно.",
